...
/* Comparison function for semaphore elements based on priority. */
static bool
                            ...
                            void *aux UNUSED)
{
    ...
    return list_entry(list_front((struct list *) &sa->semaphore.waiters), struct thread, elem)->priority >
           list_entry(list_front((struct list *) &sb->semaphore.waiters), struct thread, elem)->priority;
}
...
/* If any threads are waiting on COND (protected by LOCK), then
   ...
   An interrupt handler cannot acquire a lock, so it does not
   make sense to try to signal a condition variable from an
   interrupt handler. */
void
